window.onload = () => {
    ...
    const renderForm = () => {
      ...
      formComponent.onSubmit = (evt) => {
        ...
        /*
        Необходимо изменить: алгоритм вычисления не получает значения элементов радиогруп потому что выражение с селектором 
        "document.querySelector(`input[name=cat]`).value" всегда возвращает DOM элемент 'yes' (в соседних вызовах querySelector тоже есть эта ошибка)
...
* Ввести необходимые данные в форму.
    ...
    - Что было сделано отлично
        ...
        - В script.js есть разделение функций рендера по областям данных: рейтинги, едиторы, показ рейтинга
    ...
      - по другому использовать доступное пространство: визуальный элемент "column column_type_output" занимает очень много места на странице, 
        ...
        - На странице использован script defer="defer" и контент index.html будет сразу отрендерен браузером и виден пользователю 
            ...
            Добавить defer="defer" позднее когда в нем появится реальная необходимость
...
* Исходя из данных, введенных в форму - получить свой рейтинг.
    ...
    - Надо исправить:
        ...
        - Присвоение значнеия в свойство 'innerHTML' для вывода пользовательских значений не безопасно: Man._valueElement.innerHTML = name;
